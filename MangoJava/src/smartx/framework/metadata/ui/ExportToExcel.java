...
import smartx.framework.common.utils.WindowsCOM;
...
public class ExportToExcel {
    ...
    private Container con_parent = null;
     ...
     * @param values
     *            实际值
     * @param _ifinvokeExcel
     *            是否调用Excel程序打开该Excel,一般一客户端是用的,在服务器则不需要!!
     */
    public ExportToExcel(Container _con, String _file_name, String[] _names, String[] _types, int[] _widths,
                         Object[][] values, boolean _ifinvokeExcel) {
        this.con_parent = _con;
        this.str_path = _file_name; // 文件全路径名
        this.str_title = _names;
        this.str_types = _types;
        this.li_widths = _widths;
        this.obj_values = values;
        this.bo_ifinvokeExcel = _ifinvokeExcel; //

        initExcel();
        if (bo_ifinvokeExcel) {
            invokeExcel(); // 调出Excel直接打开Excel
        }
    }
    ...
    private void initExcel() {
        ...
        try {
            ...
            e.printStackTrace();
            JOptionPane.showMessageDialog(this.con_parent, "导出到Excel时，创建" + str_path + "文件出错！");
            return;
        }
    ...
    public void endPrintValue(){
    	try {
            ...
            JOptionPane.showMessageDialog(null, "数据导出到Excel成功!");
        } catch (IOException e) {
            e.printStackTrace();
