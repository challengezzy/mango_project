/**************************************************************************
 * $RCSfile: ExportNovaDataFrame.java,v $  $Revision: 1.2 $  $Date: 2007/05/31 07:38:25 $
 **************************************************************************/
 ...
 *
 */
public class ExportNovaDataFrame extends JFrame {
    ...
    private HashMap getPlantData() {
        if (jcb_select[2].isSelected()) {
            for (int i = 0; i < str_plantfn.length; i++) {
                ...
                vec_sqls.add("Select TEXT From user_source Where TYPE = 'FUNCTION'" + " AND NAME = '" +
                             str_plantfn[i].toUpperCase() + "' order by LINE");
            }
        }
        if (jcb_select[3].isSelected()) {
            for (int i = 0; i < str_plantpro.length; i++) {
                vec_attrs.add("PRO_" + str_plantpro[i]);
                vec_sqls.add("SELECT TEXT FROM user_source WHERE TYPE = 'PROCEDURE'" + " And NAME='" +
                             str_plantpro[i].toUpperCase() + "' ORDER BY LINE");
            }
        }
        if (jcb_select[4].isSelected()) {
            for (int i = 0; i < str_planttable.length - 2; i++) {
                ...
                vec_sqls.add("Select COLUMN_NAME, DATA_TYPE From cols Where TABLE_NAME = '" +
                             str_planttable[i].toUpperCase() + "'");
                if (str_planttable[i].equalsIgnoreCase("pub_templet_1") ||
                    str_planttable[i].equalsIgnoreCase("pub_querytemplet")) {
            ...
            vec_sqls.add(
                "select AA.name, BB.name  parentname from pub_menu AA left join pub_menu BB on AA.parentmenuid = BB.id");
        }
        if (vec_attrs.size() == 0) {
            return null;
        }
            ...
            try {
                ...
                for (int k = 0; k < hv_data.length; k++) {
                    ...
                    for (int j = 0; j < str_keys.length; j++) {
                        ...
                        if (str_planttable[i].equalsIgnoreCase("pub_menu") &&
                            str_keys[j].equalsIgnoreCase("PARENTMENUID")) {
                            str_value = str_value + ", null"; // 插入菜单表，元素的父菜单全部置为null
                        } else if (j > 0) {
                            str_value = str_value +
                                getInsertValue(hv_data[k].getStringValue(str_keys[j]), str_keys[j], hv_cols);
                        }
                    }
                ...
                if (str_planttable[i].equalsIgnoreCase("PUB_MENU")) { // 以下是处理Menu的父菜单的SQL
                    ...
                    for (int k = 0; k < hv_menu.length; k++) {
                        ...
                        String _sql =
                            "Update Pub_menu AA Set AA.PARENTMENUID = (Select BB.ID From Pub_Menu BB Where BB.NAME";
    ...
    private void writeIntoFile(String _path, String _root, HashVO _hv, HashVO[] _hv_cols, String _tab) {
        ...
        try {
            ...
            myFile.println("Delete " + _tab + "_item Where pk_" + _tab + " In " + "(Select pk_" + _tab + " From " +
                           _tab + " Where templetcode = '" + _code + "');");
            ...
            for (int k = 0; k < hv_item.length; k++) {
                ...
                for (int j = 0; j < str_keys.length; j++) {
                    ...
                    if (str_keys[j].equalsIgnoreCase("PK_" + _tab)) {
                        str_value = str_value + ", (select PK_" + _tab + "  From " + _tab + " Where TEMPLETCODE = '" +
                            _code + "')";
                    } else {
                        str_value = str_value +
                            getInsertValue(hv_item[k].getStringValue(str_keys[j]), str_keys[j], hv_cols);
                    }
                }
                myFile.print(str_temp.substring(0, str_temp.length() - 2) + ")");
                myFile.print(" values (");
...
/*******************************************************************************
 * $RCSfile: ExportNovaDataFrame.java,v $ $Revision: 1.2 $ $Date: 2007/01/30
 * 07:41:11 $
 *
 * $Log: ExportNovaDataFrame.java,v $
