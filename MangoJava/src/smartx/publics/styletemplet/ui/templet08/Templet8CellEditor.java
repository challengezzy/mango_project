/**************************************************************************
 * $RCSfile: Templet8CellEditor.java,v $  $Revision: 1.2.8.1 $  $Date: 2009/12/02 05:48:32 $
 **************************************************************************/
...
public class Templet8CellEditor extends AbstractCellEditor implements
    TableCellEditor {
    ...
    public Component getTableCellEditorComponent(JTable table, Object value,
        ...
        if (col == 1) {
            ...
            if (str.equals("主表编码")) {
                ...
                String treetableid = pritable.getRefID();                
                if (!treetableid.equals("")) {
                    prifield = new UIRefPanel_List("templet8_treefield",
                        "templet8_treefield",
                        //"select cname 列名,coltype 列类型,cname 显示列名 from col where tname='" + treetablecode + "'"
                        "select itemkey as 列名,itemtype as 列类型,itemname as 显示列名 from pub_templet_1_item where pk_pub_templet_1=(select pk_pub_templet_1 from pub_templet_1 where templetcode='"+treetableid+"')"
                        );
                } else {
                    prifield = new UIRefPanel_List("templet8_treefield",
                        "templet8_treefield",
                ...
                if (!listtableid.equals("")) {
                    subfield = new UIRefPanel_List("templet8_listfield",
                        "templet8_listfield",
                ...
                String listtableid = subtable.getRefID();
                if (!listtableid.equals("")) {
                    subfieldforeignkey = new UIRefPanel_List("templet8_listfield",
                        "templet8_listfield",
                        //"select cname as 列名,coltype 列类型,cname　显示列名 from col where tname='" + listtablecode + "'"
                        "select itemkey as 列名,itemtype as 列类型,itemname as 显示列名 from pub_templet_1_item where pk_pub_templet_1=(select pk_pub_templet_1 from pub_templet_1 where templetcode='"+listtableid+"')"
                        );
 ...
 * $RCSfile: Templet8CellEditor.java,v $  $Revision: 1.2.8.1 $  $Date: 2009/12/02 05:48:32 $
 *
 * $Log: Templet8CellEditor.java,v $
 * Revision 1.2.8.1  2009/12/02 05:48:32  wangqi
 * *** empty log message ***
 *
