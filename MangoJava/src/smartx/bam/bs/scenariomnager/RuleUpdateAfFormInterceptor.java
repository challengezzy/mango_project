 ...
 *
 */
public class RuleUpdateAfFormInterceptor implements FormInterceptor {
	...
	public void doSomething(Pub_Templet_1VO templetVO,
		...
		try{
			...
			searchSQL = "SELECT DISTINCT to_char(STREAMNAME) STREAMNAME FROM BAM_BUSINESSVIEW BV, BAM_BUSINESSSCENARIO BS, BAM_RULE BR WHERE BR.BUSINESSSCENARIOID = BS.ID AND BS.DATASOURCECODE = BV.CODE AND BS.ID=?";
			...
			String newModuleName ="rule_"+ruleCode;
			String stmtName = "stmt_"+newModuleName;
