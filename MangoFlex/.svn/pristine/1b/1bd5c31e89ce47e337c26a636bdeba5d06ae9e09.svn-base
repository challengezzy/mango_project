...
public class PodLayoutManager extends EventDispatcher
{
	...
	private function componentsCollide( obj1:DisplayObject, obj2:DisplayObject ):Boolean { 
	...
	private function onMouseMove(e:MouseEvent):void
	{
		...
		//trace("currentDragPod[x="+currentDragPod.x+";y="+currentDragPod.y+";width="+currentDragPod.width+";height="+currentDragPod.height+"]");
		for (var i:Number = 0; i < len; i++)
		{
			dragHighlightItem = DragHighlight(dragHighlightItems[i]);
			dragHighlightItem.visible = false;
			//trace("dragHighlightItem[x="+dragHighlightItem.x+";y="+dragHighlightItem.y+";width="+dragHighlightItem.width+";height="+dragHighlightItem.height+"]");
			//if (currentDragPod.hitTestObject(dragHighlightItem))
			//该方法不知为何失效，换一个
			if(componentsCollide(currentDragPod,dragHighlightItem))
			{
    ...
    private function confirmWindowClose(event:Event):void
    {
        if (event is MDIManagerEvent)
        {
            ...
            Alert.show("关闭窗口?", null, 3, null, handleAlertResponse);
        }
    }
