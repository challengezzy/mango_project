package smartx.bam.flex.modules.dqc.listener
{
	...
	public class TNNeExportListener implements ListButtonListener
	{
		...
		private var excelMTCode:String = "EXCEL_TNNE_CMPEXPORT";//excel导出格式定义
		
		public function TNNeExportListener(){
		...
		public function buttonClick(listPanel:BillListPanel):void{
			...
			if(user_region != null)
				queryCondition = queryCondition + " and city='" + user_region +"'";
			
			if(condition != null)
				queryCondition = queryCondition + " and" +condition;
			
			smartxFormService.excelExport(excelMTCode, queryCondition, "传输网元比对数据.zip");
			
			OperatingTipUtil.startOperat("生成传输网元导出文件");
		}
		...
		private function faultHandler(event:FaultEvent):void{
			OperatingTipUtil.endOperat();
			SmartXMessage.show("生成传输网元的EXCEL文件失败",SmartXMessage.MESSAGE_ERROR,event.fault.faultString);
		}
