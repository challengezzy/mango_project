package smartx.flex.modules.datatask
{
	...
	public class ViewTaskContentListener implements ListButtonListener
	{
		...
		public function ViewTaskContentListener()
		{
		...
		public function buttonClick(listPanel:BillListPanel):void
		{
			...
			querySql = "select id,taskcontent from pub_datatask t where t.id =  "+ datataskId;
		...
		private function getSimpleHashVoArrayByDSHandler(event:ResultEvent):void{
			...
			if( simpleVoArr.length > 0){
				...
				execLog = logObj.dataMap["taskcontent"];
			}else{
				SmartXMessage.show("末找对对应的任务内容！");
				return;
			}
