package smartx.bam.flex.modules.dqc.listener
{
	...
	public class GroupAnalyseMTChartClickListener implements MTChartClickListener
	{
		...
		public function GroupAnalyseMTChartClickListener()
		{
		...
		public function fcItemClick(event:FCEvent, chart:MTChartPanel):void
		{
			...
			if(!BAMUtil.isEmpty(info)){
				...
				formService.getSimpleHashVoArrayByDS(BAMConst.DATASOURCE_SMARTDQ,"select m.mtcontent content,s.entitymodelcode modelcode,s.entitycode,s.rulecode from dq_entity_statistics s,dq_entity_statistics_map m where s.mapid=m.id and s.id="+statisticsId);
			}
		}
		...
		private function getSimpleHashVoArrayByDSHandler(event:ResultEvent):void{
			...
			if(spArr != null ){
				...
				if(!BAMUtil.isEmpty(column) 
					//&& (!BAMUtil.isEmpty(value) && value != '全部' && value != 'ALL')&&value != VariableVo.IGNORE_VALUE
					){
					if(BAMUtil.isEmpty(value)){
